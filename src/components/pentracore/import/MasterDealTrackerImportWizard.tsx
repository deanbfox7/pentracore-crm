...
export default function MasterDealTrackerImportWizard() {
  ...
  const [result, setResult] = useState<{ imported_deals?: number; filename?: string | null } | null>(null)
  ...
  async function startImport() {
    setStep('importing')
    setResult(null)
    const res = await fetch('/api/pentracore/import/master-deal-tracker', {
      method: 'POST',
    ...
    setResult({ imported_deals: data.imported_deals, filename: data.filename })
    setStep('done')
  }
    ...
    <div className="space-y-4">
      {step === 'upload' && (
        ...
        >
          ...
          <div className="text-white font-semibold mb-1">Upload `MASTER_DEAL_TRACKER.csv`</div>
          <div className="text-slate-500 text-sm">We’ll map each row into `deals` + `deal_participants`.</div>
          <input
            ref={fileRef}
            type="file"
            accept=".csv,text/csv"
      ...
      {step === 'done' && (
        <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-8 text-center">
          ...
          <div className="text-slate-500 text-sm">
            Inserted/updated {result?.imported_deals ?? 0} deals.
          </div>
          <button
