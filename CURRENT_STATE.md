...
- **View readiness status** for a deal
  ...
  - Business logic:
    ...
    - **LOI exists, FCO not marked, no SPA:** "LOI/request stage - Follow up for FCO from seller"
    - **FCO marked, no SPA:** "FCO received - Prepare SPA"
  ...
  - Sections:
    ...
    - **FCO Status:** Shows FCO status with toggle button
      - Default: "⏳ Awaiting FCO from seller (Full Corporate Offer)"
      - After marking: "✅ FCO received and reviewed"
      - Button: "Mark FCO Received" / "Mark as Pending"
      - Note: "Temporary local marker — not saved to database"
    - **Awaiting:** Shows missing documents (FCO disappears from list when marked as received)
    - **Next Actions:** Suggests immediate action items based on deal stage and FCO status
    - **Readiness Status:** Summary line showing overall deal progress
  - FCO is manual local marker only (external receipt, not generated by system, not persisted to database)
  - No database schema changes
  - No new API routes
  - All state is in-memory per session
...
- ✅ FCO Received / Reviewed marker (local state, temporary, not persisted)
- ✅ Toggle button in readiness checklist: "Mark FCO Received" / "Mark as Pending"
- ✅ Updated readiness logic: FCO marked → skip "follow up for FCO" → go straight to "prepare SPA"
- ✅ FCO status section in checklist: shows current status and button to toggle
- ✅ Temporary marker label: "not saved to database" per requirements
